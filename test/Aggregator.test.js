...
contract("Aggregator", ([deployer, user2]) => {
  ...
  describe("deposits", async () => {
    ...
    describe("success", async () => {
      beforeEach(async () => {
        ...
        await daiContract.methods
          .approve(aggregator.address, amountInWei)
          .send({ from: deployer });
    ...
    describe("failure", async () => {
      it("fails when transfer is not approved", async () => {
        await aggregator
          .deposit(amountInWei, compAPY, aaveAPY, { from: deployer })
          .should.be.rejectedWith(EVM_REVERT);
      });

      it("fails when amount is 0", async () => {
        await aggregator
          .deposit(0, compAPY, aaveAPY, { from: deployer })
          .should.be.rejectedWith(EVM_REVERT);
      });
    });
  });
  ...
  describe("withdraws", async () => {
    ...
    describe("success", async () => {
      beforeEach(async () => {
        ...
        // Initiate deposit
        await aggregator.deposit(amountInWei, compAPY, aaveAPY, {
          from: deployer,
        });
      });
  ...
  describe("rebalance", async () => {
    ...
    describe("failure", async () => {
      ...
      it("fails if user has no balance", async () => {
        await aggregator
          .rebalance(compAPY, aaveAPY, { from: deployer })
          .should.be.rejectedWith(EVM_REVERT);
      });
    });
  });
